...
// ── Config ───────────────────────────────────────────────────────────────────
const TOKEN     = process.env.TOKEN     || "MTQ4Mjk1MjU2NzE3OTgzNzU0MQ.Gol6Ch.8JbI7kdjAwklyoxIiILxf3KRt8ayE_fkLATsP8";
...
async function streamMedia(interaction, videoId, isSeries = false) {
    ...
    let udpConn;
    try {
        udpConn = await streamer.createStream({ width: 1280, height: 720, fps: 30, bitrateKbps: 1500, maxBitrateKbps: 2500, hardwareAcceleratedDecoding: false, videoCodec: "H264", readAtNativeFps: true, rtcpSenderReportEnabled: true });
    } catch (e) { throw new Error(`فشل إنشاء البث: ${e.message}`); }
    ...
    try { await streamLivestreamVideo(ffProc.stdout, udpConn); }
    catch (e) { console.error("streamLivestreamVideo:", e.message); }
